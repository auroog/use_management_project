...
class UserService:
    ...
    async def _fetch_user(cls, session: AsyncSession, **filters) -> Optional[User]:
        print(f"Filters passed to query: {filters}")  # Debugging log
        query = select(User).filter_by(**filters)
        print(f"Executing query: {query}")  # Log the query structure
        result = await cls._execute_query(session, query)
        print(f"Query result: {result}")  # Log the returned results
    ...
    async def get_by_email(cls, session: AsyncSession, email: str) -> Optional[User]:
        print(f"Attempting to fetch user with email: {email}")  # Debug log
